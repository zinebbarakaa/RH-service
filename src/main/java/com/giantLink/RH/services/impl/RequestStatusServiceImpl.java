...
public class RequestStatusServiceImpl implements RequestStatusService {
    ...
    public RequestStatusResponse processHolidayRequest(Long requestStatusId) {
        ...
        Long requestHolidayId = requestStatusRepository.findRequestIdById(requestStatusId);
        Optional<RequestHoliday> requestHoliday = requestHolidayRepository.findById(requestHolidayId);
        if(requestHoliday.isPresent()){
                ...
                if(employeeHolidayBalance < requestedHolidays){
                    requestStatus.get().setStatusName(State.REFUSED);
                    ...
                    requestStatus.get().setStatusName(State.ACCEPTED);
    ...
    public List<RequestStatusResponse> getByStatusName(State state) {
        if (requestStatusRepository.findBystatusName(state).get().isEmpty()) {
            String message = messageSource.getMessage("requestStatus.notFound.message", null, "No Message", LocaleContextHolder.getLocale());
            throw new RuntimeException(message);
        } else {
            return RequestStatusMapper.INSTANCE.listToResponseList(requestStatusRepository.findBystatusName(state).get());
        }
    }
}
